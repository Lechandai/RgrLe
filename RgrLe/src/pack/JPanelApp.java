...
public class JPanelApp extends JPanel {
	// создаем перемнную типа текстовое поле
	// для результата
	JTextField txt1 = null;
	/*
	 * res, res1 переменные для операндов,
	 * используемых для расчета умножения,
	 * деления, сложения, вычитания
	 */
	double res = 0;
	double res1 = 0;
	/*
	 * op будет хранить операцию (+-/*) в виде
	 * строки
	 */
	String op = "";
	...
	public JPanelApp() {
		...
		// try отлавливает исключения(ошибки) во
		// вложенном в него коде
		try {
			/*
			 * не используем компоновку
			 * элементов, расположение элементов
			 * зададим явно
			 */
			setLayout(null);
			/*
			 * создаем объект типа текстовое поле
			 * для результата
			 */
			...
			/*
			 * далее кнопки создаются аналогично
			 * кнопке "0"
			 */
			...
			/*
			 * устанавливаем шрифт bigFont для
			 * текста, отображаемого в кнопке
			 */
			bRes.setFont(bigFont);

			/*
			 * далее класс Font использыется
			 * аналогично
			 */
			...
			JLabel label_2 = new JLabel(
					"Доход физического лица за 1 месяц");
			...
			b1.addActionListener(new ActionListener() {
				// реализуем функцию, связанную с
				// событием нажатия на кнопку
				@Override
				public void actionPerformed(ActionEvent arg1) {
					// добавляем 1 в txt1
					txt1.setText(txt1.getText() + 1);
					/*
					 * если res (первый операнд) равен 0,
					 * то конвертируем содержимое txt1
					 * в Double и записываем в res
					 */
					if (res == 0) {
						res = Double.valueOf(txt1.getText());
					} else {
						/*
						 * если res (первый операнд) не
						 * равен 0, то конвертируем
						 * содержимое txt1 в Double и
						 * записываем в res1
						 */
			 ...
			 * слушателей на следующие кнопки и
			 * реализуем реакцию на событие
			 * нажатие на кнопку
			 */
			...
			bPlus.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg1) {
					...
					txt1.setText("");
					/*
					 * в переменную операторов
					 * заносим соответствующий знак
					 * (+)
					 */
					op = "+";
				}
			...
			bRes.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					/*
					 * конвертируем res и res1 в Double,
					 * заносим в num, num1 соответственно
					 */
					double num = Double.valueOf(res);
					double num1 = Double.valueOf(res1);
					String strOp = op;
					/*
					 * создем обьект mc класса MetodCalc
					 * для расчета действия
					 */
					MetodCalc mc = new MetodCalc();
					/*
					 * вызываем у mc метод calc и
					 * передаем ему оператора и
					 * операнды
					 */
					/*
					 * результат работы метода
					 * ковертируем в String и заносим в
					 * переменныю strRes
					 */
			...
			TAXb.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) { // получаем
					...
					/*
					 * создем обьект tax класса tax для
					 * расчета налога
					 */
					tax tax = new tax();
					/*
					 * вызываем у tax метод calcTax и
					 * передаем ему необходимые
					 * значения
					 */
					/*
					 * результат работы метода
					 * конвертируем в String и заносим в
					 * переменную strRes
					 */
